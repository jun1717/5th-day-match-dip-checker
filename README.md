...
判定結果は `data/candidates.json`、`data/theme_scores.json` に保存されます。価格データがない銘柄はアプリ全体を止めず、`missing_price_data` を理由に出します。
...
損切りラインは「エントリー日の前日の安値」を使います。5日線押し目買いは「前日に5日線付近で下げ止まった」シナリオで入るため、前日安値を割り込むことはそのサポートが崩れてシナリオが否定されたことを意味します。損切りは「シナリオが崩れた地点」に置くのが合理的であり、前日安値は前日引けで確定できる具体的な数値です。

標準運用（夜に確定日足で判断して翌朝エントリー）では「エントリー日の前日」＝シグナル日なので、**画面（候補一覧・銘柄詳細）の損切りライン・株数・想定損失・リワードRはシグナル日の安値（`signalDayLow`）基準で再計算した翌朝注文用の値**を表示します。バックテスト（`stopMode: "prev-day"`）のサイジングと同じ基準なので、画面の値をそのまま注文に使えます。内部フィールド `stopLoss` / `suggestedShares` / `expectedLoss` はシグナル日前日（D-1）の安値基準のまま残してあります（バックテストの `stopLossSignal`・候補判定ロジックがこの基準で検証済みのため。判定の意味は変わりません）。
...
「毎日見るのはサイトだけ」という運用のため、自動化の静かな失敗はトップページのバナーで警告します（正常時は何も表示されません）:

- **赤（確定前データ警告）**: 平日16:40 JST以降に、表示中データが翌朝の注文に使えない状態。**この画面で翌朝の注文を組んではいけません**。GitHub Actionsのcronは2〜3時間遅れることがあるため、平常時でも一時的に出ることがあります（時間をおいて再読み込みすれば消えます）。判定は閲覧時刻に依存するためクライアント側で行い、60秒ごとに再評価します。理由は3種類:
  - `unconfirmed`: `as_of` の**時刻**が15:30 JST未満 = まだ大引け前の気配 = 表示中のバーは場中の途中値
  - `stale`: `fetched_at` がその営業日の15:30 JSTより前 = 大引け後に自動更新が1度も走っていない
  - `unknown`: どちらかが読めず判定できない

  `data/prices_as_of.json` は2項目セットで、**片方だけでは判定できません**:
  - `as_of` — Yahooの `regularMarketTime`（取引所が公表する直近の終値時刻）。**分足の最終バーを使ってはいけません** — 東証はザラバが15:25で終わり15:25〜15:30はクロージング・オークション（板寄せ）のため、1分足は15:24/15:25で止まり15:30に到達せず、確定日足を表示していても赤バナーが永久に消えなくなります（2026-08に修正済み）。
  - `fetched_at` — 価格取得を実行した時刻。`as_of` だけでは「祝日で前営業日の終値が最新」と「更新が止まって前営業日のまま」が区別できません（休場日でも `as_of` は前営業日の15:30を返すため）。cronは曜日指定（`1-5`）で祝日も走るので、`fetched_at` が当日の大引け後にあるかどうかが両者を分けます。逆にこれが無いと、朝のビルド直後にパイプラインが止まったケース（`as_of` が前営業日の15:30 = 一見確定済み）を見逃します。
- **黄（履歴保存停止警告）**: 最新スナップショット（`data/history/signals/`）がデータ日付から2営業日以上遅れている。当日の判定には影響しませんが、月次レビュー（`analyze:signals`）の基盤が欠けていくため、GitHub Actionsの実行ログを確認してください。場中ビルドでは前営業日分が最新（遅れ1営業日）が正常のため、しきい値は2営業日です。

祝日カレンダーは持っていません（平日近似）が、赤バナーは上記の `as_of` / `fetched_at` の組み合わせで判定するため休場の平日でも誤表示しません。残る誤表示は「祝日の16:40〜その日の最初の大引け後ビルドが走るまで」だけで、これは通常営業日にも同じく出る待ち時間です。判定ロジックは `lib/freshness.ts`（純関数・`tests/freshness.test.ts` で境界と祝日ケースを固定）にあります。
