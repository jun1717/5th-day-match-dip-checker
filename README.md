...
- **赤（確定前データ警告）**: 平日16:40 JST以降なのに、表示中データの終値基準（`data/prices_as_of.json`）がその営業日の大引け（15:30 JST）より前。16:30の自動更新が未反映か失敗している状態で、**この画面で翌朝の注文を組んではいけません**。GitHub Actionsのcronは2時間程度遅れることがあるため、平常時でも一時的に出ることがあります（時間をおいて再読み込みすれば消えます）。判定は閲覧時刻に依存するためクライアント側で行い、60秒ごとに再評価します。
