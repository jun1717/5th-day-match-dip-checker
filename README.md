...
- **赤（確定前データ警告）**: 平日16:40 JST以降なのに、表示中データの終値基準（`data/prices_as_of.json`）がその営業日の大引け（15:30 JST）より前。16:30の自動更新が未反映か失敗している状態で、**この画面で翌朝の注文を組んではいけません**。GitHub Actionsのcronは2〜3時間遅れることがあるため、平常時でも一時的に出ることがあります（時間をおいて再読み込みすれば消えます）。判定は閲覧時刻に依存するためクライアント側で行い、60秒ごとに再評価します。

  終値基準（`as_of`）はYahooの `regularMarketTime`（取引所が公表する直近の終値時刻）から取ります。**分足の最終バーを使ってはいけません** — 東証はザラバが15:25で終わり15:25〜15:30はクロージング・オークション（板寄せ）のため、1分足は15:24/15:25で止まり15:30に到達しません。分足由来の `as_of` は確定日足を表示していても赤バナーが永久に消えなくなります（2026-08に修正済み）。
...
祝日はカレンダー非対応（平日近似）のため、休場の平日は赤バナーが誤表示されます。休場日に翌朝の注文を組むことはないため実害はありません。判定ロジックは `lib/freshness.ts`（純関数・`tests/freshness.test.ts` で境界を固定）にあります。
